    ...
    const blogFiles = [
      {
        ...
        description:
          "Learn how to generate a JSON Web Token (JWT) in just a few seconds.",
      ...
      },
      {
        ...
        description:
          "Get started with Kubernetes and learn the basics in just a few seconds.",
        tags: ["K8s", "Node.js"],
      },
    ];
    setBlogs(blogFiles);
      ...
      <div className="py-36">
        ...
        <ul className="space-y-8 mt-12">
          {blogs.map((blog) => (
            <li
              key={blog.id}
              className="bg-[#001313] p-8 rounded-lg shadow-sm shadow-green-300 md:px-8 md:py-8 hover:shadow-green-300"
            >
              <div className="flex justify-between items-center ">
                <Link
                  to={`/blog/${blog.id}`}
                  className="text-gray-100 hover:text-green-400 text-lg"
                >
                  {blog.title}
                </Link>
